...
- `imageupdater` — optional add-on, the git-write-back half of an `argocd-image-updater` equivalent: resolves `image.track`/`image.version` against Artifact Registry (`Resolver` interface + `GCPResolver`, same interface+fake pattern as `cloudrun`/`precondition`) and, when the resolved digest differs from what's committed, rewrites *only* the `digest:` line in `service.yaml` and commits straight to the manifest repo's default branch (`githubapp.PutFile`, guarded by the blob SHA `GetFileWithSHA` returned, so a concurrent edit 409s instead of silently clobbering). Deliberately never unmarshals-then-remarshals the manifest — `yaml.Marshal` would reorder keys/drop comments/restyle quoting, turning a one-line digest bump into a whole-file diff — and re-parses the rewritten bytes as a guard that the regex hit the right line. `version` constraints (`"1"`, `"1.2"`, `"1.2.3"`) are resolved via `golang.org/x/mod/semver` by picking the highest matching tag; tags that aren't valid semver (`latest`, `main-abc123`) are silently skipped rather than erroring, since a real image repo mixes both kinds. `select.go`'s `resolve` prefers tags prefixed with the image's own name (the last path segment of `image.repository`, e.g. `a-real-etl-job-v0.323.1`) over the bare/global tag (`v1.2.3`, pushed onto every image's `:latest` on every merge by a monorepo's global release step regardless of which service actually changed) — but only once the prefixed tag's *digest* is confirmed to match what the bare tag resolves to, not by comparing version numbers (per-service and global versions are unrelated counters — a per-service tag lagging behind can't be detected by "which number is bigger"). A monorepo's per-service the-tagging-tool bump only fires on a commit touching that service's own path, so a service that only changed via a shared lib gets rebuilt/re-tagged `:latest` without its own prefixed tag moving; trusting a stale prefixed tag there commits a real regression (this happened once in production before the digest-agreement check was added — see `TestResolve_FallsBackToBareTagWhenPrefixedTagIsStale`). No bare tag at all to compare against is the one case a prefixed tag is trusted outright. Fully inert with no env var to flip: a manifest with only `digest` set is untouched, and nothing calls this at all unless some app's `service.yaml` sets `track`/`version` (which itself requires `image.repository`, enforced by `manifest.Parse`). Runs leader-gated inside `cmd/controller`'s `reconcileLoop` tick, deduped to one resolve+commit per distinct `(repo, path)` manifest (every environment an app targets shares one `service.yaml`) — any commit it makes is picked up by the *next* tick's normal fetch/diff/deploy pass, same as a hand-authored digest change; it does not itself deploy anything. One environment-agnostic manifest, one resolved digest — there's no per-environment opt-in (a `runcd.yaml`-level concern, out of scope here). The image-events Eventarc trigger (`internal/api`'s `handleImageEvent`) nudges the same `tick()` this runs inside, so an Artifact Registry push shortens the latency for both the updater and the reconcile pass through the identical `nudgeCh` path — no second trigger/handler needed
- `registry` — lists Artifact Registry docker image tags (`Client.ListTags`, cached+singleflight-coalesced per repository, `DefaultTagsCacheTTL` 30s), shared by `imageupdater` (resolving `image.track`/`image.version`) and `cloudrun` (resolving a live image deployed by tag rather than digest, see below) — one implementation, not two
- `cloudrun` — `AdminClient` interface; `GCPAdminClient` is the real Cloud Run Admin API v2 implementation (client construction coalesced via `singleflight`, using `context.WithoutCancel` so one caller's cancelled context can't spuriously fail concurrent callers). Live-state fetch resolves the container image via `resolveImageDigest`, not a bare string split: something other than RunCD may have deployed a resource by tag (e.g. `gcloud run deploy --image foo:v1`), and Cloud Run reports the image reference exactly as it was deployed — comparing a raw tag string against a desired bare digest can never match, so a tag-only reference is resolved against `registry.Client` first (via `resolveTag`/`tagResolver`, the same interface+fake pattern as everywhere else in this package) before the diff engine ever sees it. A digest-pinned reference (`@sha256:...`, what every RunCD-triggered deploy writes via `withDigest`) skips this entirely — zero added API calls for the common case; the extra lookup only ever applies to resources some other deployer still owns, not forever once RunCD syncs them.
